...
/// Create a new Cerberus instance with the given transport.
pub fn cerberus_with_transport(model: Model, transport: Box<dyn Transport>) -> Cerberus {
    Cerberus { model, transport, features: None }
}

impl Cerberus {
    /// Get the model of the Cerberus device.
    pub fn model(&self) -> Model {
        self.model
    }

    /// Get the features of the Cerberus device.
    ...
    pub fn call_raw<S: CerberusMessage>(&mut self, message: S) -> Result<ProtoMessage> {
    ...
    /// Sends a message and returns a CerberusResponse with either the expected response message,
    /// a failure or an interaction request.
    /// This method is only exported for users that want to expand the features of this library
    /// f.e. for supporting additional coins etc.
    pub fn call<'a, T, S: CerberusMessage, R: CerberusMessage>(
        &'a mut self,
        message: S,
        result_handler: Box<ResultHandler<'a, T, R>>,
    ) -> Result<CerberusResponse<'a, T, R>> {
        ...
        if resp.message_type() == R::MESSAGE_TYPE {
            ...
            match resp.message_type() {
                ...
                MessageType_PinMatrixRequest => {
                    ...
                    Ok(CerberusResponse::PinMatrixRequest(PinMatrixRequest {
                        message: req_msg,
                        client: self,
                        result_handler,
                    }))
                ...
                MessageType_PassphraseRequest => {
                    ...
                    Ok(CerberusResponse::PassphraseRequest(PassphraseRequest {
                        message: req_msg,
                        client: self,
                        result_handler,
                    }))
    ...
    ) -> Result<CerberusResponse<'_, Features, Features>> {
    ...
    pub fn ping(&mut self, message: &str) -> Result<CerberusResponse<'_, (), protos::Success>> {
        let mut req = protos::Ping::new();
        req.set_message(message.to_owned());
        self.call(req, Box::new(|_, _| Ok(())))
    }

    pub fn change_pin(&mut self, remove: bool) -> Result<CerberusResponse<'_, (), protos::Success>> {
        let mut req = protos::ChangePin::new();
        req.set_remove(remove);
        self.call(req, Box::new(|_, _| Ok(())))
    }

    pub fn wipe_device(&mut self) -> Result<CerberusResponse<'_, (), protos::Success>> {
    ...
    pub fn recover_device(
        ...
        label: String,
        dry_run: bool,
    ) -> Result<CerberusResponse<'_, (), protos::Success>> {
    ...
    pub fn reset_device(
        ...
        skip_backup: bool,
        no_backup: bool,
    ) -> Result<CerberusResponse<'_, EntropyRequest<'_>, protos::EntropyRequest>> {
    ...
    pub fn backup(&mut self) -> Result<CerberusResponse<'_, (), protos::Success>> {
    ...
    pub fn sign_identity(
        ...
        digest: Vec<u8>,
        curve: String,
    ) -> Result<CerberusResponse<'_, Vec<u8>, protos::SignedIdentity>> {
    ...
    pub fn get_ecdh_session_key(
        ...
        curve: String,
    ) -> Result<CerberusResponse<'_, protos::ECDHSessionKey, protos::ECDHSessionKey>> {
