...
def _version_from_json(json_str: str) -> translations.VersionTuple:
    version_digits = [int(v) for v in json_str.split(".")]
    if len(version_digits) < 4:
        version_digits.extend([0] * (4 - len(version_digits)))
    return t.cast(translations.VersionTuple, tuple(version_digits))
    ...
    for lang in all_languages:
        ...
        for model in ALL_MODELS:
            try:
                ...
                blob_version = _version_from_json(blob_json["header"]["version"])
                if rewrite_version:
