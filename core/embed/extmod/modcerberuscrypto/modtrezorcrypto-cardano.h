...
/// package: cerberuscrypto.cardano
...
///     CIP-3).
///     """
STATIC mp_obj_t mod_cerberuscrypto_cardano_derive_icarus(size_t n_args,
                                                       const mp_obj_t *args) {
  ...
  bool cerberus_derivation = mp_obj_is_true(args[2]);
  ...
  if (!cerberus_derivation) {
    // Exclude checksum (original Icarus spec)
    mnemonic_bytes_used = entropy_len / 8;
  } else {
    // Include checksum if it is a full byte (Cerberus bug)
    // see also https://github.com/Cerberus-Wallet/cerberus-firmware/issues/1387 and CIP-3
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(
    mod_cerberuscrypto_cardano_derive_icarus_obj, 3, 4,
    mod_cerberuscrypto_cardano_derive_icarus);

/// def from_secret(secret: bytes) -> HDNode:
///     """
///     Creates a Cardano HD node from a master secret.
///     """
STATIC mp_obj_t mod_cerberuscrypto_from_secret(mp_obj_t secret) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_cerberuscrypto_from_secret_obj,
                                 mod_cerberuscrypto_from_secret);

/// def from_seed_slip23(seed: bytes) -> HDNode:
///    """
///    Creates a Cardano HD node from a seed via SLIP-23 derivation.
///    """
STATIC mp_obj_t mod_cerberuscrypto_from_seed_slip23(mp_obj_t seed) {
  mp_buffer_info_t bufinfo;
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_cerberuscrypto_from_seed_slip23_obj,
                                 mod_cerberuscrypto_from_seed_slip23);

/// def from_seed_ledger(seed: bytes) -> HDNode:
///     """
///     Creates a Cardano HD node from a seed via Ledger derivation.
///     """
STATIC mp_obj_t mod_cerberuscrypto_from_seed_ledger(mp_obj_t seed) {
  mp_buffer_info_t bufinfo;
  mp_get_buffer_raise(seed, &bufinfo, MP_BUFFER_READ);
  if (bufinfo.len == 0) {
    mp_raise_ValueError("Invalid seed");
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_cerberuscrypto_from_seed_ledger_obj,
                                 mod_cerberuscrypto_from_seed_ledger);

STATIC const mp_rom_map_elem_t mod_cerberuscrypto_cardano_globals_table[] = {
    {MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_cardano)},
    {MP_ROM_QSTR(MP_QSTR_derive_icarus),
     MP_ROM_PTR(&mod_cerberuscrypto_cardano_derive_icarus_obj)},
    {MP_ROM_QSTR(MP_QSTR_from_secret),
     MP_ROM_PTR(&mod_cerberuscrypto_from_secret_obj)},
    {MP_ROM_QSTR(MP_QSTR_from_seed_slip23),
     MP_ROM_PTR(&mod_cerberuscrypto_from_seed_slip23_obj)},
    {MP_ROM_QSTR(MP_QSTR_from_seed_ledger),
     MP_ROM_PTR(&mod_cerberuscrypto_from_seed_ledger_obj)},
};
STATIC MP_DEFINE_CONST_DICT(mod_cerberuscrypto_cardano_globals,
                            mod_cerberuscrypto_cardano_globals_table);

STATIC const mp_obj_module_t mod_cerberuscrypto_cardano_module = {
    .base = {&mp_type_module},
    .globals = (mp_obj_dict_t *)&mod_cerberuscrypto_cardano_globals,
};
