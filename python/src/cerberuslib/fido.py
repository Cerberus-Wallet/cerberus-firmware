...
def list_credentials(client: "CerberusClient") -> "MessageType":
    return client.call(messages.WebAuthnListResidentCredentials())


@expect(messages.Success, field="message", ret_type=str)
def add_credential(client: "CerberusClient", credential_id: bytes) -> "MessageType":
    return client.call(
...
@expect(messages.Success, field="message", ret_type=str)
def remove_credential(client: "CerberusClient", index: int) -> "MessageType":
    return client.call(messages.WebAuthnRemoveResidentCredential(index=index))


@expect(messages.Success, field="message", ret_type=str)
def set_counter(client: "CerberusClient", u2f_counter: int) -> "MessageType":
...
def get_next_counter(client: "CerberusClient") -> "MessageType":
