...
def image_to_t1(filename: Path) -> bytes:
    ...
    if filename.suffix == ".toif":
        raise click.ClickException("TOIF images not supported on Cerberus One")
...
def image_to_toif(filename: Path, width: int, height: int, greyscale: bool) -> bytes:
    if filename.suffix == ".toif":
        ...
        try:
            ...
            raise click.ClickException(
                "Failed to convert image to Cerberus format"
            ) from e
...
@with_client
def pin(client: "CerberusClient", enable: Optional[bool], remove: bool) -> str:
...
@click.argument("enable", type=ChoiceType({"on": True, "off": False}), required=False)
@with_client
def wipe_code(client: "CerberusClient", enable: Optional[bool], remove: bool) -> str:
...
@with_client
def label(client: "CerberusClient", label: str) -> str:
...
@with_client
def language(
    client: "CerberusClient", path_or_url: str | None, remove: bool, display: bool | None
) -> str:
...
@with_client
def display_rotation(client: "CerberusClient", rotation: int) -> str:
    """Set display rotation.

    Configure display rotation for Cerberus Model T. The options are
...
@with_client
def auto_lock_delay(client: "CerberusClient", delay: str) -> str:
...
@with_client
def flags(client: "CerberusClient", flags: str) -> str:
    """Set device flags."""
...
@with_client
def homescreen(client: "CerberusClient", filename: str) -> str:
    """Set new homescreen.

    To revert to default homescreen, use 'cerberusctl set homescreen default'
    """
    if filename == "default":
        img = b""
    else:
...
@cli.command()
@click.option(
    "--always", is_flag=True, help='Persist the "prompt" setting across Cerberus reboots.'
)
@click.argument("level", type=ChoiceType(SAFETY_LEVELS))
@with_client
def safety_checks(
    client: "CerberusClient", always: bool, level: messages.SafetyCheckLevel
) -> str:
    """Set safety check level.

    Set to "strict" to get the full Cerberus security (default setting).
...
@with_client
def experimental_features(client: "CerberusClient", enable: bool) -> str:
...
@with_client
def passphrase_on(client: "CerberusClient", force_on_device: Optional[bool]) -> str:
    """Enable passphrase."""
...
@with_client
def passphrase_off(client: "CerberusClient") -> str:
    """Disable passphrase."""
...
@with_client
def hide_passphrase_from_host(client: "CerberusClient", hide: bool) -> str:
