...
The Bitcoin signing process is one of the most complicated workflows in the Cerberus
codebase. This is because Cerberus cannot store arbitrarily large transactions in memory,
...
In response, Cerberus will send a number of `TxRequest` messages, asking for additional
data from the host. The host is supposed to respond with a `TxAck` providing the
requested data.

Cerberus can request the following kinds of items:
...
As part of each `TxRequest` message, Cerberus can also send a chunk of the resulting
serialized transaction, and/or a signature of one of the inputs.

The flow ends when Cerberus sends a `TxRequest` with `request_type` of `TXFINISHED`.
...
Cerberus will request all inputs and outputs of the transaction to be signed, and set up
data structures that allow it to verify that the same data is sent in the following
phases.

In this phase, Cerberus will also ask the user to confirm destination addresses,
the transaction fee, metadata, and the total being sent. If the user confirms, we continue
to the next phase.

### Validation of input data

Cerberus must verify that the host does not lie about input amounts, i.e., that the
transaction total in the first phase was calculated correctly.

For this reason, Cerberus will ask the host to send each input again. It will then request
data about the referenced previous transaction: metadata, all inputs, all outputs, and
possible trailing data. This allows Cerberus to reconstruct the previous transaction and
...
Cerberus T also supports pre-signed inputs for multi-party signing. If an input has script
type `EXTERNAL` and provides a signature, Cerberus will validate the signature against the
previous transaction in this step.

### Serialization and signing

Cerberus will ask once again about every input and begin outputting a serialization of the
transaction. For every legacy (non-segwit) input, it is necessary to stream the full set
of inputs and outputs again, so that Cerberus can compute the transaction digest which it
then signs. For segwit inputs this is not necessary.

When all inputs are serialized, Cerberus will ask for every output, so that it can
serialize it, fill out change addresses, and return the full transaction.

Finally Cerberus asks again about segwit inputs to sign them and to serialize the
witnesses.

## Old versus new data structures

Originally, the `TxAck` message contained one field of type `TransactionType`. This, in
turn, contained all the possible fields that Cerberus could request:
...
If `request_details.tx_hash` is set, Cerberus is requesting data about a specified
previous transaction. If it is unset, Cerberus wants data about current transaction.

### Transaction input

Cerberus sets `request_type` to `TXINPUT`, and `request_details.tx_hash` is unset.
...
Cerberus can include inputs that it will not sign, typically because they are
...
Verified external inputs are only supported on the Cerberus T. They must either
...
their input yet (i.e., with two Cerberuss, one must sign first so that the other
...
Unverified external inputs are accepted by Cerberus only if
[safety checks](https://wiki.cerberus.uraanai.com/Using_cerberusctl_commands_with_Cerberus#Safety-checks)
are disabled on the device.

### Transaction output

Cerberus sets `request_type` to `OUTPUT`, and `request_details.tx_hash` is unset.
...
Cerberus sets `request_type` to `TXMETA`. `request_details.tx_hash` is a transaction hash,
...
Cerberus does not understand. The host must indicate the length of this extra data in the
field `extra_data_len`.

To figure out which is the extra data, the host must parse the serialized previous
transaction up to the last field understood by Cerberus. In case of Zcash, that is:
...
Cerberus sets `request_type` to `TXINPUT`. `request_details.tx_hash` is a transaction
...
Cerberus sets `request_type` to `TXOUTPUT`. `request_details.tx_hash` is a transaction
hash, matching one of the current transaction inputs.
...
understood by Cerberus. This data is not relevant for validation, but it must be included
so that Cerberus can correctly compute the previous transaction hash.

Cerberus sets `request_type` to `TXEXTRADATA`. `request_details.tx_hash` is a transaction
hash, matching one of the current transaction inputs.
...
Cerberus sets `request_type` to `TXORIGINPUT`. `request_details.tx_hash` is the
...
Cerberus sets `request_type` to `TXORIGOUTPUT`. `request_details.tx_hash` is the
transaction hash of the original transaction.
...
Cerberus sets `request_type` to `TXPAYMENTREQ`, and `request_details.tx_hash` is unset.
...
([BIP-78](https://github.com/bitcoin/bips/blob/master/bip-0078.mediawiki)). Cerberus only
...
`orig_hash` and `orig_index` fields for at least one `TxInput`. Cerberus will then
automatically request metadata about the original transaction and verify the original
signatures.

A replacement transaction in Cerberus must satisfy the following requirements:
...
In Cerberus T a set of transaction outputs can be accompanied by a payment request.
...
`payment_req_index` field in the `TxOutput` message. When Cerberus encounters the first
...
def sign_tx():
    ...
    # wait for TxAck forever, until Cerberus indicates we are finished
    while True:
        msg = receive_message()
        ...
        if msg.details.tx_hash is not None:
            # Cerberus requires data about some previous transaction
            send_response_prev(msg.request_type, msg.details)
        else:
            # Cerberus requires data about this transaction
...
representation, and Cerberus will successfully and correctly decode it into the new one.
